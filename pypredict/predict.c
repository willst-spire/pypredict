...
// TLE file contains about 5000 lines of 3-line elements, with duplicates.
// 
...
char  qthfile[50], temp[80], output[25],
  serial_port[15], reload_tle=0, netport[6],
...
char  visibility_array[DATABASE_SIZE], tracking_mode[DATABASE_SIZE];

float az_array[DATABASE_SIZE], el_array[DATABASE_SIZE], 
      long_array[DATABASE_SIZE], lat_array[DATABASE_SIZE],
      footprint_array[DATABASE_SIZE], range_array[DATABASE_SIZE], 
      altitude_array[DATABASE_SIZE], velocity_array[DATABASE_SIZE], 
      eclipse_depth_array[DATABASE_SIZE], phase_array[DATABASE_SIZE],
      squint_array[DATABASE_SIZE];

double  doppler[DATABASE_SIZE], nextevent[DATABASE_SIZE];

long  aos_array[DATABASE_SIZE], orbitnum_array[DATABASE_SIZE];
...
void usage() {
  printf("usage: doppler <required flags> [optional flags]\n"
         "      (required flags)\n"
         "    --norad-id , -n <integer>           : norad id of satellite\n"
         "    --latitude , -l <float>             : latitude of ground station\n"
         "    --longitude, -g <float>             : longitude of ground station\n"
         "    --altitude , -a <float>             : altitude of ground station\n"
         "    --tle      , -e </path/to/file.tle> : path to tle file containing norad_id\n"
         "    (optional flags)\n"
         "    --doppler  , -d                     : Output doppler shift\n"
         "    --next_pass, -p                     : Output next transit pass\n"
         "    --position , -f                     : Output current position\n"
         "    --time     , -t <epoch (s)>         : defaults to current time\n");
  exit(-1);
}

double parseDouble(char * string) {
  char * pEnd;
  double parsed_float = strtod(string, &pEnd);
  // strtod sets pEnd to the first character not parsed.
  // Should be end of string if argument is a valid float.
  if (*pEnd != 0) {
...
long parseLong(char * string) {
  char * pEnd;
  double parsed_long = strtol(string, &pEnd, 10);
  if (*pEnd != 0) {
...
void outputDoppler() {
  double doppler100=-100.0e06*((sat_range_rate*1000.0)/299792458.0);
  printf("%f\n",doppler100);
}

void outputQuickFind(int indx) {
  if (Decayed(indx,daynum)==0) {
    printf("%d\t%d\n",iel,iaz);
  } else {
    fprintf(stderr, "Decayed(%d,%f) != 0 (whatever that means)\n", indx, daynum);
    exit(-1);
  }
...
void outputQuickPredict(int indx) {
  int lastel=0;

  char aos_happens = AosHappens(indx);
  char geostationary = Geostationary(indx);
  char decayed = Decayed(indx, daynum);

  if (aos_happens && geostationary==0 && decayed==0)
  {
    /* Make Predictions */
    daynum=FindAOS();
    ...
    while (iel>=0)
    {
      printf("%.0f %4d %4d %4d %4d %4d %6ld %6ld %c\n",floor(86400.0*(3651.0+daynum)),iel,iaz,ma256,isplat,isplong,irk,rv,findsun);
      lastel=iel;
    ...
    fprintf(stderr, "AosHappens: %d, GeoStationary: %d, Decayed: %d", aos_happens, geostationary, decayed);
  }
}

void ReadTLE(char * name, char * line1, char * line2)
{
...
void ReadQTH(double lat, double lon, int alt)
{
        qth.stnlat  = lat;
        qth.stnlong = lon;
        qth.stnalt  = alt;
...
static PyObject *
pypredict_quick_predict(PyObject *self, PyObject *args, PyObject *keywds)
{
    // ground station arguments
    double lat;
    double lon;
    double alt;
    ...
    if (!PyArg_ParseTuple(args, "sssdddl", &name, &line1, &line2, &lat, &lon, &alt, &epoch)) {
        return NULL;
    }
...
static PyMethodDef predict_funcs[] = {
    {"quick_predict", (PyCFunction)pypredict_quick_predict, 
     METH_VARARGS, quick_predict_docs},
    {NULL, NULL, 0, NULL} 
};

void initpredict(void)
{
    Py_InitModule3("predict", predict_funcs,
