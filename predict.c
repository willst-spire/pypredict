	...
	if (fd!=NULL)
	{
		while (x<24 && feof(fd)==0)
		{
			...
			if (KepCheck(line1,line2) && (feof(fd)==0))
			{
				...
				if (x > 0)
				{
					fprintf(stderr, "%s has more than one TLE.\n", tlefile);
					exit(-1);
				}
		...
		if (fd!=NULL)
		{
			...
			while (strncmp(line1,"end",3)!=0 && line1[0]!='\n' && feof(fd)==0)
			{
				...
				if (match)
				{
					...
					{
						sscanf(line1,"%lf, %lf",&sat_db.alat, &sat_db.alon);
						sat_db.squintflag=1;
					}
					...
					if (entry<max_entries)
					{
						if (match)
						{
							if (strncmp(line1,"No",2)!=0)
							{
								...
								strcpy(sat_db.transponder_name[entry],line1);
							}
							else
							{
								sat_db.transponder_name[entry][0]=0;
							}
						}
						...
						if (match)
						{
							sscanf(line1,"%lf, %lf", &sat_db.uplink_start[entry], &sat_db.uplink_end[entry]);
						}
						...
						if (match)
						{
							sscanf(line1,"%lf, %lf", &sat_db.downlink_start[entry], &sat_db.downlink_end[entry]);
						}
						...
						if (match)
						{
							if (strncmp(line1,"No",2)!=0)
							{
								...
								sat_db.dayofweek[entry]=dayofweek;
							}
							else
							{
								sat_db.dayofweek[entry]=0;
							}
						}
						...
						if (match)
						{
							if (strncmp(line1,"No",2)!=0)
							{
								sscanf(line1,"%d, %d",&sat_db.phase_start[entry], &sat_db.phase_end[entry]);
							}
							else
							{
								sat_db.phase_start[entry]=0;
								sat_db.phase_end[entry]=0;
							}

							if (sat_db.uplink_start[entry]!=0.0 || sat_db.downlink_start[entry]!=0.0)
							{
								transponders++;
							}
				...
				if (match)
				{
					sat_db.transponders=transponders;
				}
		...
		if (fd!=NULL)
		{
			...
			do
			{
				if (KepCheck(str1,str2))
				{
					...
					for (i=0; (i<24 && sat.catnum!=atol(SubString(line1,2,6))); i++);
					...
					if (i!=24)
					{
						...
						if (sat.year<57)
						{
							database_year=365.25*(100.0+(float)sat.year);
						}
						else
						{
							database_year=365.25*(float)sat.year;
						}

						database_epoch=(float)sat.refepoch+database_year;
...
int Select()
{
	...
	{
		printw("\n\t[%c]: %-15s", x+'A', Abbreviate(sat.name,15));
		printw("\t[%c]: %-15s", y+'A', Abbreviate(sat.name,15));
		printw("\t[%c]: %-15s\n", z+'A', Abbreviate(sat.name,15));
	}
...
// TODO: SingleTrackWIP assumes sat_db is empty since uplink, downlink, squint, and related
//       calculations are convoluted, the db edit menu option is still marked "Coming Soon"
//       and it's never come up in our usage.  We'll add it back if there's deman.
void SingleTrackWIP() {
    int ans, oldaz=0, oldel=0, length, xponder=0,
        polarity=0, tshift, bshift;
    char    approaching=0, command[80], comsat,
        geostationary=0, aoshappens=0, decayed=0,
        visibility=0, old_visibility=0;
    double  oldtime=0.0,
    ...
    printf("Longitude(%s)  %f\n",    (io_lon=='W'?360.0-sat_lon:sat_lon), (io_lon=='W'?"W":"E"));
    printf("Azimuth        %f\n",      sat_azi);
    printf("Elevation      %f\n",      sat_ele);
    printf("Velocity       %f\n",      3600.0*sat_vel);
    printf("Footprint      %f\n",      fk);
    printf("Altitude_km    %f\n",      sat_alt);
    printf("Slant_Range_km %f\n",      sat_range);
    printf("Eclipse_Depth  %f\n",      eclipse_depth/deg2rad);
    printf("Orbital_Phase  %f\n",      256.0*(phase/twopi));
    printf("Orbital_Model  %s\n",      ephem);
    // corrected visibility (default value, based on groundstation's position, can be separately derived)
    printf("Sunlight       %d\n",      (visibility=='D' || visibility=='V'));
...
void SingleTrack(int x)
{
	...
	printw("                 Tracking: %-10sOn                                       ",Abbreviate(sat.name,9));
	printw("                                                                                 ");
...
void MultiTrack()
{
	...
	do
	{
		for (z=0; z<24; z++)
		{
			...
			if (sat.meanmo!=0.0 && Decayed(indx,0.0)!=1)
			{
				daynum=CurrentDaynum();
				PreCalc(indx);
				Calc();
		...
		if (daynum>nextcalctime)
		{
			...
			for (x=0, y=0, z=-1; x<21 && y!=3; x++)
			{
				...
				{
					mvprintw(y+21,19,"%10s on %s UTC",Abbreviate(sat.name,9),Daynum2String(aos2[x]));
...
int QuickFind(char *string, char *outputfile)
{
	...
	for (z=0; z<24; z++)
	{
		if ((strcmp(sat.name,satname)==0) || (atol(satname)==sat.catnum))
		{
			start=atol(startstr);
